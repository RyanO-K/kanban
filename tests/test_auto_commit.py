...
.kanban/ files). When such a ticket completes and the board's natural-language
...
def test_changes_are_kanban_only_true():
    paths = [".kanban/kanban-dev/36.json", ".kanban/orchestrator.py"]
    assert orch.changes_are_kanban_only(paths) is True


def test_changes_are_kanban_only_mixed_false():
    paths = [".kanban/orchestrator.py", "src/app.py"]
...
def test_completed_non_kanban_changes_still_use_branch_publish(kanban, monkeypatch):
    """Regression: a ticket that changed real source (not just .kanban/) keeps the
    existing isolated-branch publish flow — auto-commit-to-master is kanban-only."""
    pid = 9003
    p = _set_dispatched(kanban, "1", pid)
    _complete_setup(kanban, monkeypatch, pid)

    monkeypatch.setattr(orch, "discover_changed_paths", lambda kd: ["src/app.py", ".kanban/demo/1.json"])
    committed = []
    monkeypatch.setattr(orch, "commit_to_master",
...
def test_repo_dir_for_paths_kanban_only_is_kanban_repo(kanban):
    """Kanban-only changes live in the `.kanban` repo, so commit from there."""
    paths = [".kanban/demo/1.json", ".kanban/orchestrator.py"]
...
def test_commit_to_master_commits_inside_kanban_repo(kanban, monkeypatch):
    """commit_to_master must run git inside the `.kanban` repo for kanban-only work,
    ...
    assert all(c == os.path.abspath(kanban) for c in cwds), \
        "commit must run inside the .kanban repo, not the workspace root"
...
def test_agent_prompt_teaches_commit_gate():
    ...
    task = {"id": "9", "title": "Do a thing", "detail": "x", "_path": ".kanban/demo/9.json"}
